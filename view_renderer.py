...
class ViewRenderer:
    ...
    def draw(self):
        ...
        for sec_id in self.sectors:
            ...
            draw_model(ceil.model, VEC3_ZERO, 1.0, self.screen_tint)
            draw_model(floor.model, VEC3_ZERO, 1.0, self.screen_tint)

        # draw walls
        for wall in self.walls_to_draw:
            #wall = self.wall_models[wall_id]
            draw_model(wall.model, VEC3_ZERO, 1.0, self.screen_tint)

        # draw portal_mid walls from back to front
        for wall in reversed(self.mid_walls_to_draw.values()):
            draw_model(wall.model, VEC3_ZERO, 1.0, self.get_tint(wall))   
    ...
    def update_screen_tint(self):
        self.screen_tint = (
            DARK_GRAY_COLOR if self.engine.map_renderer.should_draw else WHITE_COLOR
        )        
