...
class FlatModel:
    ...
    def get_model(self):
        ...
        model.materials[0].maps[MATERIAL_MAP_DIFFUSE].texture = self.get_texture()
        return model 
    ...
    def get_polygon_mesh(self) -> Mesh:
        # triangulation
        ...
        #
        mesh.vertices = ffi.from_buffer("float []", vertices)
        mesh.indices = ffi.from_buffer("unsigned short []", indices)
        mesh.texcoords = ffi.from_buffer("float []", tex_coords)
        mesh.normals = ffi.from_buffer("float []", normals)
